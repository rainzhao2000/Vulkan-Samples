...
void Plot::request_gpu_features(vkb::PhysicalDevice &gpu)
{
...
void Plot::build_command_buffers()
{
...
void Plot::setup_descriptor_pool()
{
...
void Plot::setup_descriptor_set_layout()
{
...
void Plot::setup_descriptor_set()
{
...
void Plot::prepare_compute()
{
	...
	compute_pipeline_create_info.stage = load_shader("plot/particle_calculate.comp", VK_SHADER_STAGE_COMPUTE_BIT);
	...
	compute_pipeline_create_info.stage = load_shader("plot/particle.comp", VK_SHADER_STAGE_COMPUTE_BIT);
...
void Plot::prepare_uniform_buffers()
{
...
void Plot::update_compute_uniform_buffers(float delta_time)
{
	compute.ubo.delta_time = paused ? 0.0f : delta_time;
	compute.uniform_buffer->convert_and_update(compute.ubo);
}

void Plot::update_graphics_uniform_buffers()
{
...
bool Plot::prepare(const vkb::ApplicationOptions &options)
{
...
void Plot::render(float delta_time)
{
	if (!prepared)
	{
		return;
...
bool Plot::resize(const uint32_t width, const uint32_t height)
{
...
std::unique_ptr<vkb::VulkanSample> create_plot()
{
	return std::make_unique<Plot>();
}
